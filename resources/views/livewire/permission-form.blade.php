        ...
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <div class="p-4 space-y-4">
                <div>
                    ...
                    <x-text-input placeholder="{{ __('messages.name') }}" wire:model="permission.name" type="text" class="mt-1 block w-full"
                        autofocus autocomplete="name" />
                    <x-input-error class="mt-2" :messages="$errors->get('permission.name')" />
                </div>
                <div>
                    {{-- <x-input-label for="name" :value="__('Name')" /> --}}
                    <x-text-input placeholder="{{ __('messages.description') }}" wire:model="permission.description" type="text" class="mt-1 block w-full"
                        autofocus autocomplete="Description" />
                    <x-input-error class="mt-2" :messages="$errors->get('permission.description')" />
                </div>
                <div>
                    {{-- <x-input-label for="name" :value="__('Name')" /> --}}
                    <x-text-input placeholder="{{ __('messages.section') }}" wire:model="permission.section_name" type="text" class="mt-1 block w-full"
