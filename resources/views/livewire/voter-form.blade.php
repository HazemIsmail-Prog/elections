<x-modal wire:model="showModal" name="voter" maxWidth="sm">
    <form method="post" wire:submit.prevent="save">
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <div class="p-4 space-y-4">
                <div>
                    <x-text-input placeholder="{{ __('messages.name') }}" wire:model="voter.name" type="text" class="mt-1 block w-full"
                        autofocus autocomplete="name" />
                    <x-input-error class="mt-2" :messages="$errors->get('voter.name')" />
                </div>
            </div>
            <!-- Modal footer -->
