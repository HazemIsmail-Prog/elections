<div
    ...
    <header class="px-5 py-4 border-b border-gray-100 dark:border-gray-500 flex justify-between items-center">
        <h2 class="font-semibold text-gray-800 dark:text-white">{{ __('messages.providers') }}</h2>
        @can('providers_create')
            <x-primary-button x-data="{}" x-on:click="window.livewire.emitTo('provider-form','showModal',null)">
                {{ __('messages.add_provider') }}</x-primary-button>
        @endcan
    </header>
    <div class="p-3">
                    ...
                    @foreach ($providers as $provider)
                        <tr class=" text-center transition duration-300 ease-in-out hover:bg-neutral-100 dark:hover:bg-gray-600">
                            <td class="p-2 whitespace-nowrap">
                                <div class="text-start">{{ $provider->id }}</div>
                            </td>
                            <td class="p-2 whitespace-nowrap">
                                <div class="flex items-center">
                                    <div class="w-10 h-10 shrink-0 me-2 sm:me-3">
                                        <img class="rounded-full"
                                        src="{{ 'https://ui-avatars.com/api/?name=' . urlencode($provider->name) . '&color=7F9CF5&background=EBF4FF' }}"
                            ...
                            <td class="p-2 whitespace-nowrap">
                                <div class="text-lg text-center">
                                    <div class="flex gap-3 justify-end">
                                        @can('providers_edit')
                                            <button x-data="{}"
                                                x-on:click="window.livewire.emitTo('provider-form','showModal',{{ $provider }})">
                                                <x-svgs.edit />
                                            </button>
                                        @endcan
                                        @can('providers_delete')
                                            <button onclick="confirm('Are you sure?') || event.stopImmediatePropagation()"
                                                wire:click="delete({{ $provider }})" class="text-red-400">
                                                <x-svgs.trash />
                                            </button>
                                        @endcan
                                    </div>
