<div
    ...
    <header class="px-5 py-4 border-b border-gray-100 dark:border-gray-500 flex justify-between items-center">
        <h2 class="font-semibold text-gray-800 dark:text-white">{{ __('messages.schools') }}</h2>
        @can('schools_create')
            <x-primary-button x-data="{}" x-on:click="window.livewire.emitTo('school-form','showModal',null)">
                {{ __('messages.add_school') }}</x-primary-button>
        @endcan
    </header>
    <div class="p-3">
                    ...
                    @foreach ($schools as $school)
                        <tr class=" text-center transition duration-300 ease-in-out hover:bg-neutral-100 dark:hover:bg-gray-600">
                            <td class="p-2 whitespace-nowrap">
                                <div class="text-start">{{ $school->id }}</div>
                            </td>
                            <td class="p-2 whitespace-nowrap">
                                <div class="flex items-center">
                                    <div class="w-10 h-10 shrink-0 me-2 sm:me-3">
                                        <img class="rounded-full"
                                        src="{{ 'https://ui-avatars.com/api/?name=' . urlencode($school->name) . '&color=7F9CF5&background=EBF4FF' }}"
                                        width="40" height="40" alt="Alex Shatov" />
                                    </div>
                                    <div class="font-medium text-gray-800 dark:text-gray-200">{{ $school->name }}</div>
                                </div>
                            </td>
                            <td class="p-2 whitespace-nowrap">
                                <div class="text-lg text-center">
                                    <div class="flex gap-3 justify-end">
                                        @can('schools_edit')
                                            <button x-data="{}"
                                                x-on:click="window.livewire.emitTo('school-form','showModal',{{ $school }})">
                                                <x-svgs.edit />
                                            </button>
                                        @endcan
                                        @can('schools_delete')
                                            <button onclick="confirm('Are you sure?') || event.stopImmediatePropagation()"
                                                wire:click="delete({{ $school }})" class="text-red-400">
                                                <x-svgs.trash />
                                            </button>
                                        @endcan
                                    </div>
        ...
        <div class="px-6 py-4">{{ $schools->links() }}</div>
    </div>
</div>
